...
$build_ver = ((Get-Content $PSScriptRoot\assettocorsa\apps\lua\F1Regs\F1Regs.lua)[1] -split "`"")[1]
$build_code = ((Get-Content $PSScriptRoot\assettocorsa\apps\lua\F1Regs\F1Regs.lua)[0] -split "= ")[1]
...
(Get-Content "$PSScriptRoot\assettocorsa\apps\lua\F1Regs\F1Regs.lua") | ForEach-Object { $_ -replace "SCRIPT_VERSION_CODE =.+","SCRIPT_VERSION_CODE = $build_code"  } | Set-Content "$PSScriptRoot\assettocorsa\apps\lua\F1Regs\F1Regs.lua"
