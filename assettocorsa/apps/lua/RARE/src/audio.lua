...
DRS_BEEP:setVolume(acMainVolume * RARECONFIG.data.AUDIO.MASTER / 100 *
                       RARECONFIG.data.AUDIO.DRS_BEEP / 100)

DRS_FLAP:setSource("./assets/audio/drs-flap.wav"):setAutoPlay(false)
DRS_FLAP:setVolume(acMainVolume * RARECONFIG.data.AUDIO.MASTER / 100 *
...
local function formula1(sim, driver)
    if sim.cameraMode < 3 and sim.isWindowForeground then
        if sim.raceSessionType == ac.SessionType.Race then
            if driver.drsBeepFx and driver.car.drsAvailable and
                driver.drsAvailable then
