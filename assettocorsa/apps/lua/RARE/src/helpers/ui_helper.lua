...
function ui.inLineBulletText(label, text, space)
	...
	elseif label == "Delta" then
		...
		elseif text <= RARE_CONFIG.RULES.DRS_GAP_DELTA / 1000 and text > 0 then
			ui.textColored(text, rgbm(0, 1, 0, 1))
		else
			ui.textColored(text, rgbm(1, 0, 0, 1))
		end
	elseif string.find(label, "Tyre Life") and not string.find(label, "Limit") then
		if
			text < RARE_CONFIG.AI.AI_SINGLE_TYRE_LIFE + driver.aiTyreSingleRandom
			or (string.find(label, "Average") and text < RARE_CONFIG.AI.AI_AVG_TYRE_LIFE + driver.aiTyreAvgRandom)
		then
			ui.textColored(text .. " %", rgbm(1, 0, 0, 1))
		elseif text < RARE_CONFIG.AI.AI_AVG_TYRE_LIFE + driver.aiTyreAvgRandom then
