...
function inLineBulletText(label, text, space)
    if not space then space = 10 end
    ...
    elseif string.find(label, "Tyre Life") and not string.find(label, "Limit") then
        if text < RARECONFIG.data.AI.AI_SINGLE_TYRE_LIFE +
            driver.aiTyreSingleRandom or
            (string.find(label, "Average") and text <
                RARECONFIG.data.AI.AI_AVG_TYRE_LIFE + driver.aiTyreAvgRandom) then
            ui.textColored(text .. " %", rgbm(1, 0, 0, 1))
        elseif text < RARECONFIG.data.AI.AI_AVG_TYRE_LIFE +
            driver.aiTyreAvgRandom then
...
---@param bool boolean
---@return string
function upperBool(bool) return string.upper(tostring(bool)) end
...
function MappedConfig:set(section, key, value, hexFormat)
    ...
    setTimeout(function()
        log('Saving updated [' .. section .. '][' .. key .. ']: ' ..
                tostring(value))
        self.ini:setAndSave(section, key,
                            hexFormat and
                                string.format('0x%x', self.data[section][key]) or
...
function slider(cfg, section, key, from, to, mult, isbool, format, tooltip,
                preprocess)
    ...
    local curValue = ui.slider('##' .. section .. key, mult < 0 and -mult /
                                   cfg.data[section][key] or
                                   cfg.data[section][key] * mult, from, to,
                               format)
    if tooltip and (ui.itemHovered() or ui.itemActive()) then
        (type(tooltip) == 'function' and ui.tooltip or ui.setTooltip)(tooltip)
    end
    if preprocess then curValue = preprocess(curValue) end
    ui.sameLine(0, 4)
    local changed = math.abs(cfg.original[section][key] -
                                 (mult < 0 and -mult / curValue or curValue /
                                     mult)) > 0.0001
    if ui.button('##r' .. section .. key, vec2(20, 20),
                 changed and ui.ButtonFlags.None or ui.ButtonFlags.Disabled) then
        if isbool then
            curValue = cfg.original[section][key]
        else
            curValue = mult < 0 and -mult / cfg.original[section][key] or
                           cfg.original[section][key] * mult
        end
    end
    ...
    if ui.itemHovered() then
        ...
        if isbool then
            ...
            v = string.format(format:match('%%.+'),
                              mult < 0 and -mult / cfg.original[section][key] or
                                  cfg.original[section][key] * mult)

        end
        ui.setTooltip(string.format(changed and
                                        'Click to restore original value: %s' or
                                        'Original value: %s', v))
    end
...
function resetTrackSurfaces()
    local config = ac.INIConfig.load(ac.getFolder(ac.FolderID.ACApps) ..
                                         "/lua/RARE/settings.ini",
