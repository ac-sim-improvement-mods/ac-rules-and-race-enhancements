function debugMenu(sim, rc, error)
	...
	ui.treeNode("[PIT STOPS]", ui.TreeNodeFlags.DefaultOpen and ui.TreeNodeFlags.Framed, function()
		...
		inLineBulletText("Last Pit Time", math.round(driver.pitlaneTime, 2), space)
		inLineBulletText("Last Pit Stop Time", math.round(driver.pitstopTime, 2), space)
	end)
	...
	ui.treeNode("[TYRES]", ui.TreeNodeFlags.DefaultOpen and ui.TreeNodeFlags.Framed, function()
		...
		inLineBulletText("Compound Change", upperBool(driver.tyreCompoundChange), space)
		inLineBulletText(
			"Compounds Available",
			"H:"
		...
		inLineBulletText("Current Compound", ac.getTyresName(driver.car.index, driver.car.compoundIndex), space)
		inLineBulletText("Next Compound", ac.getTyresName(driver.index, driver.tyreCompoundNext), space)
		inLineBulletText("Start Compound", ac.getTyresName(driver.index, driver.tyreCompoundStart), space)
		...
		inLineBulletText("Tyre Life Average", math.round(100 - (avg_tyre_wear * 100), 1), space)
		if driver.car.isAIControlled then
			inLineBulletText("Pitting New Tyres", upperBool(driver.aiPitting), space)
		end
		inLineBulletText("Tyre Life [FL]", math.round(100 - (driver.car.wheels[0].tyreWear * 100), 1), space)
		inLineBulletText("Tyre Life [RL]", math.round(100 - (driver.car.wheels[2].tyreWear * 100), 1), space)
		inLineBulletText("Tyre Life [FR]", math.round(100 - (driver.car.wheels[1].tyreWear * 100), 1), space)
		inLineBulletText("Tyre Life [RR]", math.round(100 - (driver.car.wheels[3].tyreWear * 100), 1), space)
	end)
	...
	if driver.car.kersPresent then
		ui.treeNode("[HYBRID SYSTEMS]", ui.TreeNodeFlags.DefaultOpen and ui.TreeNodeFlags.Framed, function()
			...
			inLineBulletText("ERS Charge", math.round(driver.car.kersCharge * 100, 2) .. " %", space)
			inLineBulletText(
				"ERS Spent",
		...
		ui.treeNode("[DRS]", ui.TreeNodeFlags.DefaultOpen and ui.TreeNodeFlags.Framed, function()
			if driver.car.drsPresent then
				if not driver.car.isInPitlane then
					...
					inLineBulletText("Detection Line", tostring(getDetectionLineDistanceM(driver)) .. " m", space)
					inLineBulletText("Start Line", tostring(getStartLineDistanceM(driver)) .. " m", space)
	...
	ui.treeNode("[WEATHER]", ui.TreeNodeFlags.DefaultOpen and ui.TreeNodeFlags.Framed, function()
		...
		inLineBulletText("Track Wetness", math.round(sim.rainWetness * 1000, 2) .. "%", space)
		inLineBulletText("Track Puddles", math.round(sim.rainWater * 1000, 2) .. "%", space)
		inLineBulletText("Total Wetness", math.round(totalWetness, 2) .. "%", space)
