...
function settingsMenu(sim)
	...
	if sim.isInMainMenu then
		...
		ui.drawRectFilled(
			vec2(380, 30),
			vec2(420, 60),
			(ac.isWindowOpen("rare") and physics.allowed()) and rgbm(0, 1, 0, 0.5) or rgbm(1, 0, 0, 0.5)
		)
	...
	ui.tabBar("settingstabbar", ui.TabBarFlags.None, function()
		ui.tabItem("RULES", ui.TabItemFlags.None, function()
			...
			slider(
				...
				1,
				true,
				RARECONFIG.data.RULES.RACE_REFUELING == 1 and "Race Refueling: ENABLED" or "Race Refueling: DISABLED",
		...
		ui.tabItem("AI", ui.TabItemFlags.None, function()
			...
			slider(
				...
				1,
				true,
				RARECONFIG.data.AI.AI_RELATIVE_LEVEL == 1 and "Relative AI Level %.0f%%" or "Relative AI Level %.0f%%",
			...
			slider(
				...
				1,
				true,
				RARECONFIG.data.AI.AI_MGUK_CONTROL == 1 and "AI Dynamic MGUK: ENABLED" or "AI Dynamic MGUK: DISABLED",
			...
			if RARECONFIG.data.AI.AI_FORCE_PIT_TYRES == 1 then
				...
				if not sim.isInMainMenu then
					if driver.car.isAIControlled then
						...
						if
							ui.button("FORCE ALL AI TO PIT NOW", vec2(ui.windowWidth() - 77, 25), ui.ButtonFlags.None)
						then
							for i = 0, #DRIVERS do
