...
local raceControlBlockImage = ui.decodeImage(io.loadFromZip(ac.findFile("assets\\img\\img.zip"), "fia_block.png"))
local raceControlBlockImageSize = ui.imageSize(raceControlBlockImage)
local raceControlBannerImage = ui.decodeImage(io.loadFromZip(ac.findFile("assets\\img\\img.zip"), "fia_banner.png"))
local raceControlBannerImageSize = ui.imageSize(raceControlBannerImage)

local function drawRaceControl(text)
	ui.beginScale()
	ui.setCursorX(1700)
	ui.setCursorY(200)
	ui.image(raceControlBlockImage, vec2(100, 75), true)
	ui.setCursorX(1800)
	ui.setCursorY(200)
	ui.image(raceControlBannerImage, vec2(750, 75))
